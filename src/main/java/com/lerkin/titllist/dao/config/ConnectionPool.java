...
class ConnectionPool {
	...
	public ConnectionPool() {
		...
		int poolSize = JDBCData.POOL_SIZE;
		connectionQueue = new ArrayBlockingQueue<>(poolSize);
		busyConnection = new LinkedList<>();

		try {
			Class.forName(JDBCData.DRIVER);
			for (int i = 0; i < poolSize; i++) {
				Connection connection = DriverManager.getConnection(JDBCData.URL, JDBCData.USER, JDBCData.PASSWORD);
	...
	public Connection takeConnection() throws InterruptedException {
		...
		Connection taken = connectionQueue.take();
		busyConnection.add(taken);
		return taken;
	}
	...
	private class PooledConnection implements Connection {
		...
		public void close() throws SQLException {
			...
			boolean offered = connectionQueue.offer(this);
			if (!offered) {
				throw new RuntimeException("Cannot return connection to pool");
			}
			busyConnection.remove(this);
